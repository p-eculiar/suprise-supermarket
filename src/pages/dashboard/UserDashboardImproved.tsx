    ...
    const fetchCategories = async () => {
      try {
        ...
        if (!catErr && catRows && catRows.length > 0) {
          ...
          const categoriesWithIcons = catRows.map((cat: any) => ({
            name: cat.name,
            icon: iconMap[cat.name.toLowerCase()] || '🛍️'
          }));
          
          console.log('✅ Categories from categories table:', categoriesWithIcons);
        ...
        if (!prodErr && prodRows) {
          ...
          console.log('✅ Final categories from products table:', categoriesWithIcons);
          setCategories(categoriesWithIcons);
        }
      } catch (error) {
          ...
          <StatsContainer>
            ...
            <StatCard>
              ...
              <StatInfo>
                ...
                <StatLabel>Recent Orders</StatLabel>
              </StatInfo>
            </StatCard>
            <StatCard>
              <StatIcon bg="#4A6D5D">
              ...
              <StatInfo>
                ...
                <StatLabel>Cart Total</StatLabel>
              </StatInfo>
            </StatCard>
          </StatsContainer>
        </WelcomeContent>
            ...
            <OrderList>
              {loadingOrders ? (
                ...
                userOrders.map(order => (
                  <OrderItem key={order.id} onClick={() => navigate(`/dashboard/history`)}>
                    <OrderInfo>
...
const CategoryCard = styled.div`
  ...
  &:hover {
    ...
    color: white;
    transform: translateY(-5px);
    box-shadow: 0 8px 20px rgba(108, 154, 127, 0.3);
  }
