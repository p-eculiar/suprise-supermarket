...
import { FiMessageSquare, FiStar, FiRefreshCw, FiCheck, FiClock, FiUser, FiAlertCircle } from 'react-icons/fi';
import { format } from 'date-fns';

// Explicitly export the icons to ensure they're properly defined
const IconFiMessageSquare = FiMessageSquare;
const IconFiStar = FiStar;
const IconFiRefreshCw = FiRefreshCw;
const IconFiCheck = FiCheck;
const IconFiClock = FiClock;
const IconFiUser = FiUser;
const IconFiAlertCircle = FiAlertCircle;
...
const EnhancedRealtimeData: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // Load feedback without joining with any tables to avoid permission issues
      ...
      // Enhance feedback with basic user info
      const enhancedFeedback = feedbackData?.map((item: any) => ({
        ...item,
        user_email: item.user_id || 'Unknown User',
        user_name: item.user_id ? `User ${item.user_id.substring(0, 8)}` : 'Unknown User'
      })) || [];

      console.log('✅ Loaded feedback:', enhancedFeedback.length, 'items');
      setFeedback(enhancedFeedback);

      // Load messages without joining with any tables to avoid permission issues
      ...
      // Enhance messages with basic user info
      const enhancedMessages = messagesData?.map((item: any) => ({
        ...item,
        user_email: item.user_id || 'Unknown User',
        user_name: item.user_id ? `User ${item.user_id.substring(0, 8)}` : 'Unknown User'
      })) || [];

      console.log('✅ Loaded messages:', enhancedMessages.length, 'items');
  ...
  // Real-time updates for feedback with better error handling
  useRealtime({
    table: 'feedback',
    events: ['INSERT', 'UPDATE', 'DELETE'],
    onEvent: (payload: any) => {
      console.log('🔔 Feedback real-time event:', payload.eventType, payload.new?.id);
      loadData();
    },
    channelName: 'admin-feedback-realtime-enhanced'
  });

  // Real-time updates for messages with better error handling
  useRealtime({
    table: 'messages',
    events: ['INSERT', 'UPDATE', 'DELETE'],
    onEvent: (payload: any) => {
      console.log('🔔 Messages real-time event:', payload.eventType, payload.new?.id);
      loadData();
    },
    channelName: 'admin-messages-realtime-enhanced'
  });

  // Initial data load
  useEffect(() => {
    console.log('🚀 Initializing RealtimeData component');
    loadData();
  }, [loadData]);
  ...
  const handleMarkAsResponded = async (type: 'feedback' | 'message', id: string) => {
    try {
      ...
      setError(error instanceof Error ? error.message : 'Failed to mark as responded');
    }
  };
  ...
  if (loading && feedback.length === 0 && messages.length === 0) {
    return (
      <Container>
        <LoadingState>
          <Spinner />
  ...
  return (
    <Container>
      <Header>
        <TitleSection>
          <Title>Enhanced Real-Time Data</Title>
      ...
      <Tabs>
        ...
        >
          <IconFiStar /> Feedback ({feedback.length})
        </TabButton>
        <TabButton 
          $active={activeTab === 'messages'} 
          onClick={() => setActiveTab('messages')}
        >
          <IconFiMessageSquare /> Messages ({messages.length})
        </TabButton>
      </Tabs>
                  ...
                  <DetailRow>
                    ...
                    <DetailValue>
                      <Rating $color={getRatingColor((selectedItem as Feedback).rating)}>
                        <IconFiStar /> {(selectedItem as Feedback).rating} / 5
                      </Rating>
                    </DetailValue>
                  </DetailRow>
                  <DetailRow>
...
export default EnhancedRealtimeData;
...
const ConnectionStatus = styled.div<{ $status: string }>`
  ...
  background: ${props => {
    switch (props.$status) {
      case 'connected': return '#27AE6020';
      case 'connecting': return '#F39C1220';
      case 'disconnected': return '#E74C3C20';
      case 'error': return '#E74C3C20';
      default: return '#95A5A620';
    }
  }};
  color: ${props => {
    switch (props.$status) {
