// Script to completely remove all duplicate products, keeping only one copy of each
...
async function completeDuplicateRemoval() {
  try {
    console.log('🔍 Completely removing all duplicate products...');
    ...
    for (const group of duplicates) {
      ...
      // Keep the first one (smallest ID) and remove the rest
    ...
    if (!finalError) {
      ...
      if (remainingDuplicates.length > 0) {
        remainingDuplicates.slice(0, 20).forEach(([key, count]) => {
          const [name, price, category] = key.split('|');
          console.log(`   - "${name}" ($${price}, ${category}) appears ${count} times`);
        });
        
        if (remainingDuplicates.length > 20) {
          console.log(`   ... and ${remainingDuplicates.length - 20} more`);
        }
      } else {
